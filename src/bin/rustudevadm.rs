...
fn handle_settle(args: &SettleArgs) -> anyhow::Result<()> {
    ...
    // Let the daemon drain its netlink fd while it handles the request. A
    // queue-file-only check can return before the kernel delivers an event
    // generated by the preceding trigger write.
    if Path::new("/run/udev/control").exists() {
        let command = format!("settle={}", args.timeout);
        match send_udevd_control_with_timeout(&command, timeout + Duration::from_secs(2)) {
            Ok(()) => return Ok(()),
            Err(_error) if !Path::new("/run/udev/control").exists() => {}
            Err(error) => {
                if args.quiet {
                    return Ok(());
                }
    ...
    loop {
        ...
        if !queue_file.exists() || fs::metadata(queue_file).map_or(true, |meta| meta.len() == 0) {
            // A daemon-less fallback still needs a quiet interval so a
            // delayed kernel netlink event cannot escape the settle window.
            if start.elapsed() >= Duration::from_millis(100) {
                thread::sleep(Duration::from_millis(100));
                if !queue_file.exists()
                    || fs::metadata(queue_file).map_or(true, |meta| meta.len() == 0)
                {
                    return Ok(());
                }
            }
...
fn send_udevd_control(command: &str) -> anyhow::Result<()> {
    send_udevd_control_with_timeout(command, Duration::from_secs(2))
}

fn send_udevd_control_with_timeout(command: &str, timeout: Duration) -> anyhow::Result<()> {

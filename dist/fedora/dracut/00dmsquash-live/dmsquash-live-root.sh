...
if [ -f "$livedev" ]; then
    ...
    if [ "$livedev_fstype" = squashfs ]; then
        ...
        if ! mount -n -t "$livedev_fstype" -o "${liverw:-ro}" \
            "$livedev" /run/initramfs/live; then
            die "Failed to mount block device of live image"
            exit 1
        fi
    elif [ -x /usr/bin/ntfs-3g ]; then
        (exec -a @ntfs-3g ntfs-3g -o "${liverw:-ro}" "$livedev" \
            /run/initramfs/live) | vwarn
    else
        die "Failed to mount live image: missing NTFS support"
        exit 1
    fi
fi
...
do_live_overlay() {
    ...
    if [ -z "$setup" ] && [ -n "$devspec" ] && [ -n "$pathspec" ] \
        && [ -n "$overlay" ]; then
        ...
        if [ -f /run/initramfs/overlayfs$pathspec ] \
            && [ -w /run/initramfs/overlayfs$pathspec ]; then
            OVERLAY_LOOPDEV=$(losetup -f --show ${readonly_overlay:+-r} \
            ...
            if [ -z "$oltype" ] || [ "$oltype" = DM_snapshot_cow ]; then
                ...
                mount -n -t "$oltype" ${readonly_overlay:+-r} \
                    "$OVERLAY_LOOPDEV" /run/initramfs/overlayfs
                if [ -d /run/initramfs/overlayfs/overlayfs ] \
                    && [ -d /run/initramfs/overlayfs/ovlwork ]; then
                    ln -s /run/initramfs/overlayfs/overlayfs \
                        /run/overlayfs${readonly_overlay:+-r}
                    ln -s /run/initramfs/overlayfs/ovlwork \
                        /run/ovlwork${readonly_overlay:+-r}
            ...
            ln -s /run/initramfs/overlayfs$pathspec \
                /run/overlayfs${readonly_overlay:+-r}
            ln -s /run/initramfs/overlayfs$pathspec/../ovlwork \
                /run/ovlwork${readonly_overlay:+-r}
            if [ -z "$overlayfs" ] && [ -n "$DRACUT_SYSTEMD" ]; then
                reloadsysrootmountunit=':>/xor_overlayfs;'
            fi
    ...
    if [ -n "$overlayfs" ] && ! modprobe overlay; then
    ...
    if [ -z "$setup" ] || [ -n "$readonly_overlay" ]; then
        if [ -n "$setup" ]; then
            ...
            [ -z "$m" ] && m='Unable to find a persistent overlay; using a temporary one.'
        ...
        if [ -n "$overlayfs" ]; then
            if [ -n "$readonly_overlay" ] && ! [ -h /run/overlayfs-r ]; then
                ...
                [ -n "$DRACUT_SYSTEMD" ] \
                    && reloadsysrootmountunit=':>/xor_readonly;'
            fi
        else
            dd if=/dev/null of=/overlay bs=1024 count=1 \
                seek=$((overlay_size * 1024)) 2>/dev/null
    ...
    if [ -z "$overlayfs" ]; then
        if [ -n "$readonly_overlay" ] && [ -n "$OVERLAY_LOOPDEV" ]; then
            echo 0 "$sz" snapshot "$BASE_LOOPDEV" "$OVERLAY_LOOPDEV" P 8 \
                | dmsetup create --readonly live-ro
    ...
    if [ -n "$thin_snapshot" ]; then
        ...
        dd if=/dev/null of=/run/initramfs/thin-overlay/meta bs=1b count=1 \
            seek="$thin_meta_sz" 2>/dev/null
        dd if=/dev/null of=/run/initramfs/thin-overlay/data bs=1b count=1 \
            seek="$thin_data_sz" 2>/dev/null
        ...
        echo 0 "$thin_data_sz" thin-pool "$THIN_META_LOOPDEV" \
            "$THIN_DATA_LOOPDEV" 1024 1024 | dmsetup create live-overlay-pool
        dmsetup message /dev/mapper/live-overlay-pool 0 "create_thin 0"
        echo 0 "$thin_data_sz" thin /dev/mapper/live-overlay-pool 0 "$base" \
            | dmsetup create live-rw
    elif [ -z "$overlayfs" ]; then
        echo 0 "$sz" snapshot "$base" "$over" PO 8 \
            | dmsetup create live-rw
    fi

    if [ -z "$overlayfs" ]; then
        echo 0 "$sz" linear "$BASE_LOOPDEV" 0 \
...
if [ -e "$SQUASHED" ]; then
    ...
    elif [ -d /run/initramfs/squashfs/proc ] \
        || { [ -d /run/initramfs/squashfs/etc ] \
            && [ -d /run/initramfs/squashfs/usr ]; }; then
        FSIMG=$SQUASHED
        if [ -z "$overlayfs" ] && [ -n "$DRACUT_SYSTEMD" ]; then
            reloadsysrootmountunit=':>/xor_overlayfs;'
        fi
...
if [ -n "$FSIMG" ]; then
    ...
    readonly_base=1
    if { [ -z "$SQUASHED" ] && [ -n "$live_ram" ] \
        && [ -z "$overlayfs" ]; } || [ -n "$writable_fsimg" ] \

...
#
# Host/candidate v261 Manager oracle for the startup completion timestamps.
...
for property in FinishTimestamp FinishTimestampMonotonic; do
    printf '%s\n' "$xml" | grep -F \
        ...
        printf '%s\n' "manager finish timestamp oracle: SKIP (host manager lacks $property)"
        exit 0
    }
...
attempt=0
while [ "$attempt" -lt 100 ]; do
    finish_realtime=$(get_property FinishTimestamp)
    finish_monotonic=$(get_property FinishTimestampMonotonic)
    if [ "$finish_realtime" -gt 0 ] && [ "$finish_monotonic" -gt 0 ]; then
        break
    fi
    attempt=$((attempt + 1))
    sleep 0.05
...
userspace_monotonic=$(get_property UserspaceTimestampMonotonic)
[ "$finish_monotonic" -ge "$userspace_monotonic" ]
[ "$finish_realtime" = "$(get_property FinishTimestamp)" ]
[ "$finish_monotonic" = "$(get_property FinishTimestampMonotonic)" ]

printf '%s\n' 'manager finish timestamp oracle: PASS'

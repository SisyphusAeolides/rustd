...
# 338/338 is non-negotiable before systemd-libs can leave the RPM transaction.
...
grep -Fxq 'required=338' "$abi" || fail 'ABI voucher does not require 338 symbols'
grep -Fxq 'supported=338' "$abi" || fail 'ABI voucher does not certify 338 symbols'

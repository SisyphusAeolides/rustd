#!/usr/bin/env python3
"""Assemble independently produced RustD machine evidence fail-closed."""
...
def main() -> int:
    ...
    if not validator.is_file():
        raise ValueError(f"machine evidence validator not found: {validator}")

    combined_fd, combined_name = tempfile.mkstemp(
        prefix="rustd-machine-evidence-", suffix=".jsonl", dir=output.parent
    )
    normalized_fd, normalized_name = tempfile.mkstemp(
        prefix="rustd-machine-evidence-normalized-", suffix=".jsonl", dir=output.parent
    )
    ...
    try:
        ...
        with os.fdopen(normalized_fd, "wb", closefd=True) as destination:
            result = subprocess.run(
                [
                    ...
                    str(combined),
                    "--expected-rustd-sha",
                    options.expected_rustd_sha,
                    "--expected-resolved-sha",
                    options.expected_resolved_sha,
                ],
                stdout=destination,
                check=False,
            )
        ...
        if result.returncode != 0:
            raise ValueError("combined evidence failed the production machine-evidence validator")

        # Refuse an unsafe existing destination rather than replacing something
        # the caller may not own or may have intentionally linked elsewhere.
        if output.exists():
            info = output.lstat()
...
if __name__ == "__main__":
    try:
        ...
        print(f"RustD machine evidence assembly: {error}", file=sys.stderr)
